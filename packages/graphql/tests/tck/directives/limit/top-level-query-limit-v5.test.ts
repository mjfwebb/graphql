...
describe("top level query limits version 5", () => {
    let typeDefs: string;
    let neoSchema: Neo4jGraphQL;

    beforeAll(() => {
        typeDefs = /* GraphQL */ `
            # type Tag @limit(default: 10, max: 100) @node(labels: ["Tag", "UNIVERSAL"]) {
            type Tag @limit(default: 10, max: 100) @node(labels: ["Tag", "UNIVERSAL"]) {
                id: Int
                name: String!
            }
    ...
    test("extras query should have supplied limit of 14", async () => {
        ...
        expect(formatCypher(result.cypher)).toMatchInlineSnapshot(`
            ...
            RETURN this { nestedGetTagsByNamesCustomLimit: var1 } AS this"
        `);
