...
export default function createProjectionAndParams({
    ...
    function reducer(res: Res, field: ResolveTree): Res {
        ...
        if (relationField) {
            ...
            if (relationField.interface || relationField.union) {
                ...
                res.projection.push(new Cypher.Raw((env) => `${alias}: ${compileCypher(subqueryReturnAlias, env)}`));
        ...
        if (aggregationFieldProjection) {
            ...
            res.projection.push(
                new Cypher.Raw((env) => `${alias}: ${compileCypher(aggregationFieldProjection.projectionCypher, env)}`)
            );
            return res;
        }
        ...
        if (connectionField) {
            ...
            res.projection.push(new Cypher.Raw((env) => `${field.alias}: ${compileCypher(returnVariable, env)}`));
        ...
        if (pointField) {
            ...
            res.projection.push(new Cypher.Raw((env) => `${field.alias}: ${compileCypher(pointExpr, env)}`));
            ...
            res.projection.push(new Cypher.Raw((env) => `${field.alias}: ${compileCypher(datetimeExpr, env)}`));
        } else {
            ...
            if (alias !== field.name || dbFieldName !== field.name || literalElements) {
                res.projection.push(
                    new Cypher.Raw((env) => `${alias}: ${compileCypher(varName.property(dbFieldName), env)}`)
                );
            } else {
                res.projection.push(new Cypher.Raw(`.${dbFieldName}`));
            }
        }
    ...
    const { params, subqueriesBeforeSort, subqueries, predicates, projection } = Object.values(mergedFields).reduce(
        reducer,
        {
            projection: resolveType
                ? [
                      new Cypher.Raw(`__resolveType: "${node.name}"`),
                      new Cypher.Raw((env) => `__id: id(${compileCypher(varName, env)})`),
                  ]
                : [],
            params: {},
            subqueries: [],
    ...
    const projectionCypher = new Cypher.Raw((env) => {
